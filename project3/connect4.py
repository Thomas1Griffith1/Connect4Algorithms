...
#John Anghinetti and Thomas Griffith
...
def minimax(player, board, depth_limit):
    ...
    ###############################################################################
    def value(p, b, d):
        if d == 0 or board.terminal() is True:
            if board.terminal() is True:
                if board.who_wins() == max_player:
                    return 100000000, None
                else:
                    return -100000000, None
            else:
                return evaluate(p, b), None

        elif p == max_player:
            return max_value(p, b, d)
        else:
            return min_value(p, b, d)

    def max_value(p, b, depth):
        next_player = board.PLAYER2 if p == board.PLAYER1 else board.PLAYER1
        childs = get_child_boards(p, b)

        val = -math.inf
        place = None
        for c in childs:
            new_score = value(next_player, c[1], depth - 1)[0]
            if new_score > val:
                val = new_score
                place = c[0]
...
def alphabeta(player, board, depth_limit):
    ...
    def value(p, b, d, a, be):
        if d == 0 or board.terminal() is True:
            ...
            return max_value(p, b, d, a, be)
        else:
            return min_value(p, b, d, a, be)

    def max_value(p, b, depth, a, be):
        next_player = board.PLAYER2 if p == board.PLAYER1 else board.PLAYER1
        childs = get_child_boards(p, b)
        ...
        place = None
        for c in childs:
            new_score = value(next_player, c[1], depth - 1, a, be)[0]
            if new_score > val:
                val = new_score
                place = c[0]
            a = max(a, val)
